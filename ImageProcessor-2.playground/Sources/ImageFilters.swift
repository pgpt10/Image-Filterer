...
public extension UIImage
{
    ...
    public func applyDefaultFilter(filterType : FilterType) -> UIImage
    {
        ...
        case .Negative:
            ImageFilter.negativeImage(rgbaImage: &rgbaImage, withParameter: 200)
        case .Brightness:
        ...
        case .Reddish:
            ImageFilter.reddishImage(rgbaImage: &rgbaImage, withParameter: 3)
        case .Greenish:
            ImageFilter.greenishImage(rgbaImage: &rgbaImage, withParameter: 3)
        case .Blueish:
            ImageFilter.blueishImage(rgbaImage: &rgbaImage, withParameter: 3)
        }
...
struct ImageFilter
{
    ...
    {
        let param = Int(max(155, min(255, parameter)))
        for x in 0..<rgbaImage.width
        {
            for y in 0..<rgbaImage.height
            {
    ...
    static func reddishImage( rgbaImage : inout RGBAImage, withParameter parameter : Int)
    {
        ...
        for x in 0..<rgbaImage.width
        {
            for y in 0..<rgbaImage.height
            {
                ...
                let redDifference = Int(pixel.red) - rgbaImage.avgRed
                if redDifference > 0
                {
                    pixel.red = UInt8(max(0, min(255, rgbaImage.avgRed + redDifference * param)))
                    rgbaImage.pixels[index] = pixel
                }
            }
        }
    ...
    static func greenishImage( rgbaImage : inout RGBAImage, withParameter parameter : Int)
    {
        ...
        for x in 0..<rgbaImage.width
        {
            for y in 0..<rgbaImage.height
            {
                ...
                let greenDifference = Int(pixel.green) - rgbaImage.avgGreen
                if greenDifference > 0
                {
                    pixel.green = UInt8(max(0, min(255, rgbaImage.avgGreen + greenDifference * param)))
                    rgbaImage.pixels[index] = pixel
                }
            }
        }
    ...
    static func blueishImage( rgbaImage : inout RGBAImage, withParameter parameter : Int)
    {
        ...
        for x in 0..<rgbaImage.width
        {
            for y in 0..<rgbaImage.height
            {
                ...
                let blueDifference = Int(pixel.blue) - rgbaImage.avgBlue
                if blueDifference > 0
                {
                    pixel.blue = UInt8(max(0, min(255, rgbaImage.avgBlue + blueDifference *  param)))
                    rgbaImage.pixels[index] = pixel
                }
            }
        }
